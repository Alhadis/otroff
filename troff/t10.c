...
ptinit()
{
	...
	*/
	...
	filebase = setbrk(dev.filesize + 3*(dev.biggestfont+1) + dev.nchtab + 128 - 32);	/* room for file + 1 font */
	...
	fontbase[0]->nwfont = dev.biggestfont + 1;
...
newpage(n)	/* called at end of each output page (we hope) */
{
	...
	fdprintf(ptid, "p%d\n", n);	/* new page */
	for (i = 0; i <= nfonts; i++)
		if (fontbase[i] && fontbase[i]->namefont[0]) {
			/* long names not terminated in dev.h; 10 is size! */
			strncpy(temp, fontbase[i]->namefont, 10);
			fdprintf(ptid, "x font %d %s\n", i, temp);
		}
	ptps();
	ptfont();
